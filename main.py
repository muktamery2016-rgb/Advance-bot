...
from TEAMXLG import DEcwHisPErMsG_pb2 , MajoRLoGinrEs_pb2 , PorTs_pb2 , MajoRLoGinrEq_pb2 , sQ_pb2 , Team_msg_pb2, RemoveFriend_Req_pb2, GetFriend_Res_pb2, spam_request_pb2, devxt_count_pb2, dev_generator_pb2, kyro_title_pb2, room_join_pb2
from protobuf_decoder.protobuf_decoder import Parser
from TEAMxC4 import * ; from xHeaders import *
...
region = 'BD'
WHITELISTED_UIDS = {
    "TANBIR", # don't change this text
    "15518888800"
}
# ADMIN INFO FUNCTION FOR ADMIN COMMAND 
ADMIN_UID = "15518888800"
server2 = "BD"
key2 = "@yashapis"
...
uid=4166952162,password=EADFF58BC414901536B1C4D817412EAFCB62BB729860C657D9A7FD63B5DA1157
...
def load_credentials_from_file(filename="dev.txt"):
    """
    Load UID and password from dev.txt file
    """
    try:
        if not os.path.exists(filename):
...
📱 Player will see:
"TANBIR wants to join your team"
...
async def generate_guest_accounts(count=1, name="TANBIR", password_prefix="FF"):
...
async def generate_and_save_accounts(count, name="TANBIR", password_prefix="FF"):
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]MARUF YT [00FF00]___ MARUF_YT_444 ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @MARUF_YT_444\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
def send_tiktok_info(username):
    ...
    try:
        ...
        if user.get("credit") != "XANAFLEGACY":
...
[C][B][00FFFF]━━━━━━━━━━━
[C][B][FFFFFF]Developer     : MARUF HASAN 
"""
    # Send main info
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]XANAF LEGACY [00FF00]!!
"""
...
def talk_with_ai(question):
    url = f"http://localhost:5001/ask?key=TEAMXLEGACY&question={question}"
    res = requests.get(url)
    if res.status_code == 200:
        data = res.json()
...
async def EncRypTMajoRLoGin(open_id, access_token):
    ...
    #major_login.analytics_detail = b"F\u2665\u25baAUY\n\u263a\u2193Q\u263cCZS\u25acVA:V[P  #[FPg\u25c4S \\Wb"
    major_login.loading_time = 13094
    major_login.release_channel = "android"
    #major_login.extra_info = "KqsHTxzK/nz6kdF/gcCWLRalpH+FqH/RxMvcjGF9mCx8AlBlvpaxlVUXuV1++lLI6j8jznKPd1NCm+zNLXONrupu6ZE="
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="TEAMXLG", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="TEAMXLG", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde XANAF LEGACY ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="TEAMXLG", title_id=title_id)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        if (not WHITELIST_ONLY) or (str(squad_owner) in WHITELISTED_UIDS):
                            ...
                            try:
                                ...
                                bot_uid = 13777734363
                                message_text = f" Can't accept Your request Talk to TEAMXLG"
...
[b][c][00FF00]Wʟᴄ Tᴏ MARUF BOT HERE
❀️ [B][C][00FFFF]╭[00FFFF]─[00FFFF]╮[00FFFF]╔═══════╗
[C][B][00FFFF]│[FFFFFF]ꚠ[00FFFF]│[00FFFF]║[FFFFFF]@MARUF 444[00FFFF]║
...
[00FFFF]━━━━━━━━━━━━[FF69B4]"""
                ...
                if senthi == True:
                    ...
                    def get_random_color(): return "_" 
                    message = """[C][B][FF00FF]═⚡ Hᴇʟʟᴏ, <Uꜱᴇʀ> 
[b][c][00FF00]Wʟᴄ Tᴏ Venom Community Bᴏᴛ Hᴇʀᴇ 
❀️ [B][C][00FFFF]╭[00FFFF]─[00FFFF]╮[00FFFF]╔═══════╗
[C][B][00FFFF]│[FFFFFF]ꚠ[00FFFF]│[00FFFF]║[FFFFFF]@mr_zihad[00FFFF]║
[C][B][00FFFF]╰[00FFFF]─[00FFFF]╯[00FFFF]╚═══════╝
[00FFFF]━━━━━━━━━━━━[FF69B4]"""
                        # In your auto-join (Old Handler) code, find this line:
...
"""
                                ...
                                try:
                                    url = f"https://rizerxghostbd.onrender.com/x/{teamcode}/XANAF_LEGACY"
...
# NEW LAG COMMAND (AUTO STOP AFTER 10 SECONDS)
                        if inPuTMsG.startswith('/XANAFLEGACYerror'):
...
# NEW ATTACK COMMAND (AUTO STOP AFTER 1 SECOND)
                        if inPuTMsG.strip().startswith('/XANAFLEGACYerror '):
                        ...
                        if inPuTMsG.strip().startswith('/XANAFLEGACY'):
                            print('Processing wave message command')
                          
                            parts = inPuTMsG.strip().split()
    
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ Usage: /XANAFLEGACY (message) [repeats=5]\n"
                                error_msg += f"[FFFFFF]Example: /XANAFLEGACY hello 3\n"
...
# GLOBAL BLOCKED NAMES
                        BLOCKED_NAMES = ["xanaf", "teamx", "omegax", "temaxmegagivway"]  # Protected names
...
# ==================== /luck COMMAND ====================
                        if inPuTMsG.strip() == "/luck":
                            ...
                            admin_message = (
                                f"[C][B][00FFFF]╔════════════════════╗\n"
                                f"║  ╭──────────────╮\n"
                                f"║  │[FFFF00] MARUF HASAN VIP BOT          │\n"
                                f"║  ╰──────────────╯\n"
                                f"║\n"
                                f"║ [FF69B4]✨ {selected_message}\n"
                                f"║ ────────────────\n"
                                f"║ ⚡ [FF00FF] MARUF  HASAN VIP BOT ⚡\n"
                                f"╚════════════════════╝"
                            )
...
# In TcPChaT function, update /status command:
                        ...
                        # FIXED XAANF TCP HELP MENU SYSTEM - Now detects commands properly
                        # IMPROVED XAANF TCP HELP MENU SYSTEM - AUTOMATIC MULTI-PART
                        # IMPROVED XANAF TCP HELP MENU SYSTEM - TREE STYLE FORMAT
                        if inPuTMsG.strip().lower() in ("help","/help","hi","hello","commands"):
                            print(f"Help command detected from UID: {uid}")

                            p1 = """[C][B][00FFFF]════════════
Venom Community HELP MENU 1 
════════════

[FF0000]1 [FFFFFF]Show Help Manu
❀️ [00FFFF]/help 

[00FF00]2 [FFFFFF]Get Player Info
❀️ [00FFFF]/info [uid]

[FFFF00]3 [FFFFFF]Get YouTube Info
❀️ [00FFFF]/yt [name]

[FFA500]4 [FFFFFF]Room Id If Player In Room
❀️ [00FFFF]/roomid [uid]

[00FFFF]5 [FFFFFF]Send Likes
❀️ [00FFFF]/like [uid]
"""
                            ...
                            p2 = """[C][B][FF00FF]════════════
Venom Community HELP MENU 2
════════════

[FF4500]6 [FFFFFF]Leave From Group
❀️ [FF00FF]/exit

[FF0000]7 [FFFFFF]Join Team
❀️ [FF00FF]Just Send Team Code

[00FF00]8 [FFFFFF]Account Ban Check
❀️ [FF00FF]/check [uid]

[FFFF00]9 [FFFFFF]Make 5 Player Group
❀️ [FF00FF]/5

[FFA500]10 [FFFFFF]Make 3 Player Group
❀️ [FF00FF]/3
"""
                            ...
                            p3 = """[C][B][00FF00]════════════
Venom Community HELP MENU 3
════════════

[FF0000]11 [FFFFFF]Make 6 Player Group
❀️ [00FF00]/6

[00FF00]12 [FFFFFF]Ban Player
❀️ [00FF00]/ban [uid]

[FFFF00]13 [FFFFFF]Unban Player 
❀️ [00FF00]/unban [uid]

[FFA500]14 [FFFFFF]Try Your Luck
❀️ [00FF00]/luck

[00FFFF]15 [FFFFFF]Gali Any Friend 
❀️ [00FF00]/gali [name]
"""
                            ...
                            p4 = """[C][B][00BFFF]════════════
Venom Community HELP MENU 4
════════════

[FF00FF]16 [FFFFFF]Praise Any Player
❀️ [00BFFF]/praise [name]

[FF4500]17 [FFFFFF]Love Msg To Person 
❀️ [00BFFF]/love [name]

[FF0000]18 [FFFFFF]Chat With AI
❀️ [00BFFF]/ai

[00FF00]19 [FFFFFF]Add Bot In Friend 
❀️ [00BFFF]/add [uid]

[FFFF00]20 [FFFFFF]Remove From Friend 
❀️ [00BFFF]/remove [uid]
"""
                            ...
                            p5 = """[C][B][32CD32]════════════
Venom Community HELP MENU 5
════════════

[FF0000]21 [FFFFFF]Dice Roll of Ludo
❀️ [32CD32]/dice

[00FF00]22 [FFFFFF]Coin Flip Head or Tell
❀️ [32CD32]/coin

[FFFF00]23 [FFFFFF]Evo Emote Cycle 
❀️ [32CD32]@evos

[FFA500]24 [FFFFFF]Stop Evo Emote Cycle 
❀️ [32CD32]@sevos

[00FFFF]25 [FFFFFF]Play Emote
❀️ [32CD32]/e [emote]
"""
                            ...
                            p6 = """[C][B][FF1493]════════════
Venom Community HELP MENU 6
════════════

[FF00FF]26 [FFFFFF]Quick 25x Emote
❀️ [FF1493]/fast [uids] [emote]

[FF4500]27 [FFFFFF]Emote Spam
❀️ [FF1493]/es [uids] [emote]

[FF0000]28 [FFFFFF]Emote Without Bot 
❀️ [FF1493]/em [teamcode] [uids] [emote

[00FF00]29 [FFFFFF]Show Sticker From Bot
❀️ [FF1493]/sticker

[FFFF00]30 [FFFFFF]Start Match
❀️ [FF1493]/start [teamcode]
"""
                            await safe_send_message(response.Data.chat_type,p6,uid,chat_id,key,iv)
                            await asyncio.sleep(0.2)

                            p7 = """[C][B][FFA500]════════════
Venom Community HELP MENU 7
════════════

[FF0000]31 [FFFFFF]Invite Player
❀️ [FFA500]/inv [uid]

[00FF00]32 [FFFFFF]Details Level Info
❀️ [FFA500]/level [uid]

[FFFF00]33 [FFFFFF]Get & Copy Player Bio
❀️ [FFA500]/bio [uid]

[FFA500]34 [FFFFFF]See Event Info
❀️ [FFA500]/all_event [region]

[00FFFF]35 [FFFFFF]Kick Player
❀️ [FFA500]/kick [uid]
"""
                            ...
                            p8 = """[C][B][FFD700]════════════
Venom Community HELP MENU 8 
════════════

[FF0000]36 [FFFFFF]Get Guild Info
❀️ [FFD700]/guild [guild_id]

[00FF00]37 [FFFFFF]Spam Join Requests 
❀️ [FFD700]/spam_join [uid] [count]

[FFFF00]38 [FFFFFF]Spam Friend Requests 
❀️ [FFD700]/spam_req [uid]

[FFA500]39 [FFFFFF]Spam Invite 
❀️ [FFD700]/spm_inv [uid]

[00FFFF]40 [FFFFFF]Spam Room Invite 
❀️ [FFD700]/spam_room [uid]
"""
                            await safe_send_message(response.Data.chat_type,p8,uid,chat_id,key,iv)
                            await asyncio.sleep(0.2)

                            p9 = """[C][B][FF69B4]════════════
Venom Community HELP MENU 9
════════════

[FF0000]41 [FFFFFF]See Bangla Dhadha & Ans
❀️ [FF69B4]/dhadha

[00FF00]42 [FFFFFF]Play Emote Using Id
❀️ [FF69B4]/play [uid] [emote_id]

[FFFF00]43 [FFFFFF]Freeze Player Using Emote
❀️ [FF69B4]/freeze [uid]

[FFA500]44 [FFFFFF]Set Welcome Emote
❀️ [FF69B4]/welcomeemote [emote_id]

[00FFFF]45 [FFFFFF]Get 10000 Visit In Account 
❀️ [FF69B4]/visit [uid]
"""
                            ...
                            p10 = """[C][B][00FA9A]════════════
Venom Community HELP MENU 10 
════════════

[FF0000]46 [FFFFFF]Get Bundle List
❀️ [00FA9A]/bundle

[00FF00]47 [FFFFFF]Equip Bundle To Bot
❀️ [00FA9A]/bundle [name]

[FFFF00]48 [FFFFFF]Reply of Noob
❀️ [00FA9A]noob

[FFA500]49 [FFFFFF]Show Rare Titles
❀️ [00FA9A]/title

[00FFFF]50 [FFFFFF]See All Rare Emote List
❀️ [00FA9A]/emotes
"""
                            ...
                            p11 = """[C][B][8A2BE2]════════════
Venom Community HELP MENU 11 
════════════

[FF0000]51 [FFFFFF]Message Spam
❀️ [8A2BE2]/ms [msg] [count]

[00FF00]52 [FFFFFF]Wave Message Spam
❀️ [8A2BE2]/mg [msg] [count]

[FFFF00]53 [FFFFFF]Direct Private Message 
❀️ [8A2BE2]/dm [uid]

[FFA500]54 [FFFFFF]Get Player Status
❀️ [8A2BE2]/status [uid]

[00FFFF]55 [FFFFFF]Couut Emote Play
❀️ [8A2BE2]/p [uid] [emote] [number]
"""
                            await safe_send_message(response.Data.chat_type,p11,uid,chat_id,key,iv)
                            await asyncio.sleep(0.2)

                            p12 = """[C][B][00FF7F]════════════
Venom Community HELP MENU 12 
════════════

[FF0000]56 [FFFFFF]Craftland Badge Spam
❀️ [00FF7F]/s1 [uid]

[00FF00]57 [FFFFFF]V Badge Spam
❀️ [00FF7F]/s2 [uid]

[FFFF00]58 [FFFFFF]Modarator Badge Spam
❀️ [00FF7F]/s3 [uid]

[FFA500]59 [FFFFFF]Old V Badge Spam
❀️ [00FF7F]/s4 [uid]

[00FFFF]60 [FFFFFF]Pro Badge Spam
❀️ [00FF7F]/s5 [uid]
"""
                            ...
                            p13 = """[C][B][32CD32]════════════
Venom Community HELP MENU 13
════════════

[FF0000]61 [FFFFFF]Ghost Lobby In Team 
❀️ [FF1493]/ghost [teamcode]

[00FF00]62 [FFFFFF]See Ship In Person 
❀️ [FF1493]/ship [person1] [person2]

[FFFF00]63 [FFFFFF]Random D A R E
❀️ [FF1493]/dare

[FFA500]64 [FFFFFF]Rate Anything 
❀️ [FF1493]/rate [subject]

[00FFFF]65 [FFFFFF]See Advice Game Releted
❀️ [FF1493]/advice
"""
                            ...
                            p14 = """[C][B][8A2BE2]════════════
Venom Community HELP MENU 14
════════════

[FF0000]66 [FFFFFF]Random Emote
❀️ [8A2BE2]Just Send a Emoji or title

[00FF00]67 [FFFFFF]See Admin Info
❀️ [8A2BE2]/admin

[FFFF00]68 [FFFFFF]Direct Private Message 
❀️ [8A2BE2]/dm [uid]

[FFA500]69 [FFFFFF]Emote Like Spam
❀️ [8A2BE2]/es [uid]

[00FFFF]70 [FFFFFF]Count Emote Play
❀️ [8A2BE2]/p [uid] [emote] [number]
"""
                            ...
                            p15 = """[C][B][00FF7F]════════════
Venom Community HELP MENU 15
════════════

[FF0000]56 [FFFFFF]Get Animation List
❀️ [00FF7F]/animation 

[00FF00]57 [FFFFFF]Play Animation 
❀️ [00FF7F]/animation [anim]

[FFFF00]58 [FFFFFF]Look Changer List
❀️ [00FF7F]/look

[FFA500]59 [FFFFFF]Play Look Changer 
❀️ [00FF7F]/look [lookchanger]

[00FFFF]60 [FFFFFF]Auto Look Changes 
❀️ [00FF7F]Auto After Accept Invite
"""
                            await safe_send_message(response.Data.chat_type,p15,uid,chat_id,key,iv)
                            await asyncio.sleep(0.2)
                            
                            p16 = """[C][B][FF6347]════════════
Venom Community HELP MENU 16
════════════

[FFA500]61 [FFFFFF]Lag Attack (Fast Join-Leave)
❀️ [FF6347]/lag [teamcode]

[00FFFF]63 [FFFFFF]Stop Lag Attack
❀️ [FF6347]/stop lag

[FF1493]64 [FFFFFF]Reject Spam (150 packets)
❀️ [FF6347]/reject [uid]

[FF1493]65 [FFFFFF]Stop Reject Spam
❀️ [FF6347]/reject_stop
...
[B][C][FF69B4] গিল্ড এর নিয়ম প্রতি সপ্তাহে ❀️ 10,000 গ্লরি দিতে হবে
[FFFF00]════════════"""
...
[FFFFFF]YOUTUBE & [00FFFF]TELEGRAM 

[FF00FF] ❀️TELIGRAM:
[FFFF00] ㅤ@MARUF HASAN

[FFFFFF]❀️ Subscribe ᴀɴᴅ Sᴜᴘᴘᴏʀᴛ: 
[00FFFF] ㅤ@MARUF HASAN
